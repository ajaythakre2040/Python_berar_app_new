...
from ems.serializers import MenuSerializer
...
class GetAllParentMenuView(APIView):
    permission_classes = [IsAuthenticated, IsTokenValid]

    def get(self, request):
        parent_menus = Menu.objects.filter(menu_id=0, deleted_at__isnull=True)
        if parent_menus.exists():
            serializer = MenuSerializer(parent_menus, many=True)
            return Response(
                {
                    "success": True,
        ...
        return Response(
            {
                "success": True,
                "message": "No parent menus available.",
                "data": [],
            },
            status=status.HTTP_200_OK,
        )
...
class GetMenuByPortalIdView(APIView):
    permission_classes = [IsAuthenticated, IsTokenValid]

    def get(self, request, portal_id):
        menus = Menu.objects.filter(portal_id=portal_id, deleted_at__isnull=True)
        if menus.exists():
            serializer = MenuSerializer(menus, many=True)
            return Response(
                {
                    "success": True,
                    "message": f"Menus for portal ID {portal_id} retrieved successfully.",
                    "data": serializer.data,
                },
                status=status.HTTP_200_OK,
            )
        return Response(
            {
                "success": True,
                "message": f"No menus found for portal ID {portal_id}.",
                "data": [],
            },
            status=status.HTTP_200_OK,
        )
